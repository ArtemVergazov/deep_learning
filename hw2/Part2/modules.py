...
class CustomMultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        for i in range(self.num_heads):
            ...
            res = q @ k.transpose(-2, -1) / self.norm_coeff
...
class StepLRWithWarmup(torch.optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        ...
        if 0 < self.last_epoch <= self.warmup_epochs:
            coef = (self.base_lrs[0] - self.warmup_lr_init) / self.warmup_epochs
            return [coef * self.last_epoch + self.warmup_lr_init for _ in self.optimizer.param_groups]
...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        ...
        random_tensor = (torch.rand(shape, dtype=x.dtype) > self.drop_prob).to(device)
...
class TransformerEncoder(nn.Module):
    def __init__(self, embedding_dim, num_heads, mlp_size, dropout=0.1, attention_dropout=0.1,
        ...
        self.attention = torch.nn.MultiheadAttention(embedding_dim, num_heads, dropout=attention_dropout, batch_first=True)
