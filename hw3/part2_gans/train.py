...
class GAN(pl.LightningModule):
    def __init__(
        ...
        if self.truncation_trick:
            ...
            self.val_noise.data = torch.tensor(stats.truncnorm.rvs(
                -1, 1,
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        gen_out = self(noise, labels)
        fake_dis_out = self.dis(gen_out, labels)

        if optimizer_idx == 0:  # gen loss
            loss = self.loss(fake_dis_out)
        
        if optimizer_idx == 1:  # dis loss
            real_dis_out = self.dis(imgs, labels)
            loss = self.loss(fake_dis_out, real_dis_out)
